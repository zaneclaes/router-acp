...
`router-acp` is a single ACP-compatible agent process — an ACP session router over `(agent, model)` **candidates**, with bounded in-session delegation. Your ACP client connects to it as if it were any coding agent; downstream, the router holds connections open to one or more seat-authenticated ACP adapters: [`@agentclientprotocol/claude-agent-acp`](https://github.com/agentclientprotocol/claude-agent-acp), [`@agentclientprotocol/codex-acp`](https://github.com/agentclientprotocol/codex-acp), xAI's own CLI [`@xai-official/grok`](https://www.npmjs.com/package/@xai-official/grok) (it speaks ACP natively via `grok agent stdio` — no separate adapter needed; Grok's `_x.ai/ask_user_question` is translated to ACP `elicitation/create` when the client advertises form elicitation, and Grok's `terminal/create` with `/bin/bash -lc '…'` stuffed into `command` is split into executable + args before the client `spawn()`s it), and Moonshot's [`kimi-cli`](https://github.com/MoonshotAI/kimi-cli) (likewise native via `kimi acp`):
...
- **`static`** — always use one named candidate (`router.candidate`, or config's `routers.static.candidate`). A signed-out target returns ACP `auth_required`; another unroutable target is an actionable error. Neither silently substitutes unless `allow_fallback: true`.
...
| `auto_upgrade.enabled` | `false` | Opt in to auto-switching a pinned session up to a more capable model when confidence drops. Off by default: a quality verdict should not re-pin a live session (explicit `[router: switch=…]`, skill routes, and cordon failover always work). |
...
| `model_version_pins` | `{}` | Version pinning: `agent/model-id` → `agent/model-id`. Whenever candidate resolution lands on a **key** — the strategy's winner, an explicit `candidate=`/`switch=` naming the key, a skill route, `routers.static.candidate` — the session pins to the **target** instead, and the routing disclosure names the target (plus a `version pin: <key> → <target>` note) so the user sees which model actually served the turn. Single hop: a target that is itself a key is not followed. Both sides must be declared models of the **same** agent or the config fails to load; the target is normally an `auto_eligible: false` legacy entry. Naming the target directly is unaffected. Every *stated* reference — a config file's `routers.static.candidate`, a fully-qualified `agent/model:` shorthand, a `delegate_task` `hints.candidate` — is resolved through this map, so config and prompts keep naming the family's stable default id. |
...
| `agents[].models[]` | – | `id` (must exactly match the downstream selector's value for `config-option`, unless `downstream_id` is set), optional `display_name`, `api_model` (provider model id for proxy rewrites; defaults to `id`), `downstream_id`, `cost_rank` (1 = cheapest/least scarce), `auto_eligible`, and optional `pricing`. |
| `agents[].models[].auto_eligible` | `true` | `false` removes the candidate from every **automatic** selection pool — the `auto`/`pareto-code` strategies, escalation and demotion targets, pre-classifier evaluators, orchestration planner/reviewer globs, skill routes, failover, delegate scoping, and per-request proxy routing — while it stays advertised in `router.candidate` (flagged `auto_eligible: false` in the option's `_meta.router_acp`), selectable by name via `[router: candidate=…]`/`switch=`, and valid as a `model_version_pins` target. This is how a pinned legacy version stays reachable on purpose and never by accident. |
| `agents[].models[].downstream_id` | `id` | The value sent to a `config-option` adapter's model selector, when it differs from `id`. Adapters advertise a small alias set and refuse anything else, so a candidate whose `id` is a bare provider model id (`claude-opus-4-6`) is dropped at probe time as "not offered by downstream model selector"; pointing `downstream_id` at a known-good alias keeps it selectable while `api_model` pins the real wire model. |
