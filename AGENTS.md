...
| `src/pre_classifier.rs` | composable **LLM pre-classifier** (`pre_classifier.*`): one cheap tool-less ACP evaluation of the first prompt returns structured `routing` (authoritative task class/complexity, supersedes the static classifier) + `orchestrate` + host dimensions (e.g. Kory's `ui_planning`). **Mandatory when enabled**: NO wall-clock timeout on the LLM call (core infra — it must run; a slow model finishes, a dead one EOFs to an error); a failed evaluator is run through `classify_failure`+`apply_failure` (cordon) and **failed over** to the next eligible evaluator; if none can classify the turn **hard-fails** (`respond_with_error`, `preclass_done` stays false) — it never silently falls back to the heuristic and never loops. Interrupt is client cancellation (`RequestCancellation::run_until_cancelled`), not a timer |
...
- **Proactive usage cordons** (`cordon.*` + per-agent `usage_source`;
  ...
  **fail-open** (any poll/token/parse error → no cordon; the reactive per-agent
  ...
  box-wide — falling back to its on-disk rollout-JSONL snapshot (last-known,
  undocumented → parse fails open; the reactive cordon backstops both). Codex writes one
...
- **Per-request LLM proxy** (`llm_proxy.*` + `agents[].llm_proxy`, off by
  ...
  Codex ChatGPT OAuth normally prefers WebSocket and bypasses
  `OPENAI_BASE_URL`; `codex_chatgpt_provider: true` installs a custom HTTP
